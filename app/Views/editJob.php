...
<header class="container" style="background-image:url('<?=base_url('img/eyejpg.jpg')?>'); border-bottom:solid 1px white; background-position: center; background-repeat:no-repeat; background-color: white;
    ...
    <div class="col text-right">
        <h3 style="margin-top:35px">Edit Job</h3>
    </div>
</header>
<hr>
<?php
...
<input method="POST" type="hidden" name="idJob" value="<?=$job->idJob?> "> 
    ...
    <div class="row">
        <div class="col-4 offset-4">
            <div class="form-group">
                <label> Tarefa: </label>
                <input name="jobName" type="text" placeholder="Job Name" class="form-control" value="<?=$job->job?>" required>
            </div><br>
            <div>
                <a class="btn btn-secondary" href="<?= site_url('main')?>">Voltar</a>

                <input type="submit" value="Atualizar" class="btn btn btn-dark" style="float:right;">
            </div>
        </div>
    </div>
</div>
